...
def _do_fold(train, test, i, K, X, Y):
    
    import pandas as pd
    from os import system, path

    system("mkdir fold{0}".format(i))
...
def _extract_feature_importances(CONFIG, fold, e, colnames):
    ...
    plt.savefig(path.join(CONFIG.project_path, fold, "feature_importance_plot_{0}.png".format(CONFIG.experiment_name)))

    pd.DataFrame.from_dict({"feature" : feature_names, "importance" : importances[indices]}).to_csv(path.join(CONFIG.project_path, fold, "feature_importances_{0}.csv".format(CONFIG.experiment_name)))